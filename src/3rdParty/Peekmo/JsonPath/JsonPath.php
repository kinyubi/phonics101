...
class JsonPath
{
    private string $resultType = 'Value';
     ...
     *
     * @return array|bool
     * @throws Exception if $obj not an array
     */
    public function jsonPath($obj, $expr, $args = null)
    {
        ...
        $this->obj = $obj;
        if ($expr && $obj && ('VALUE' == $this->resultType || 'PATH' == $this->resultType)) {
    ...
    private function trace($expr, $val, $path)
    {
        if ('' !== $expr) {
            ...
            if (is_array($val) && array_key_exists($loc, $val)) {
                ...
                $this->walk($loc, $x, $val, $path, [&$this, '_callback_03']);
            } elseif ('..' === $loc) {
                $this->trace($x, $val, $path);
                $this->walk($loc, $x, $val, $path, [&$this, '_callback_04']);
            } elseif (preg_match("/^\(.*?\)$/", $loc)) { // [(expr)]
                $this->trace($this->evalx($loc, $val, substr($path, strrpos($path, ';') + 1)).';'.$x, $val, $path);
            } elseif (preg_match("/^\?\(.*?\)$/", $loc)) { // [?(expr)]
                $this->walk($loc, $x, $val, $path, [&$this, '_callback_05']);
            } elseif (preg_match(
                '/^(-?[0-9]*):(-?[0-9]*):?(-?[0-9]*)$/',
                $loc
    ...
    private function evalx($x, $v, $vname = null)
    {
        ...
        if (false === $res) {
            echo '(jsonPath) SyntaxError: '.$expr;
        } else {
            return $name;
        }
    }
    ...
    {
        if ($p) {
            array_push($this->result, ('PATH' == $this->resultType ? $this->asPath($p) : $v));
        }
